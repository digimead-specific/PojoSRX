...
public class Util {
    ...
    /**
     * Converts a module identifier to a bundle identifier. Module IDs are typically <tt>&lt;bundle-id&gt;.&lt;revision&gt;</tt>; this
    ...
    /**
     * <p> This is a simple utility class that attempts to load the named class using the class loader of the supplied class or the class
     * loader of one of its super classes or their implemented interfaces. This is necessary during service registration to test whether a
     * given service object implements its declared service interfaces. </p> <p> To perform this test, the framework must try to load the
     * classes associated with the declared service interfaces, so it must choose a class loader. The class loader of the registering bundle
     * cannot be used, since this disallows third parties to register service on behalf of another bundle. Consequently, the class loader of
     * the service object must be used. However, this is also not sufficient since the class loader of the service object may not have
     * direct access to the class in question. </p> <p> The service object's class loader may not have direct access to its service
     * interface if it extends a super class from another bundle which implements the service interface from an imported bundle or if it
     * implements an extension of the service interface from another bundle which imports the base interface from another bundle. In these
     * cases, the service object's class loader only has access to the super class's class or the extended service interface, respectively,
     * but not to the actual service interface. </p> <p> Thus, it is necessary to not only try to load the service interface class from the
     * service object's class loader, but from the class loaders of any interfaces it implements and the class loaders of all super classes.
     * </p>
     *
    ...
    /**
     * This method determines if the requesting bundle is able to cast the specified service reference based on class visibility rules of
     * the underlying modules.
     *
     * @param requester The bundle requesting the service.
     * @param ref The service in question.
     * @return <tt>true</tt> if the requesting bundle is able to case the service object to a known type.
     *
     */
    ...
    /**
     * <p> This method performs property variable substitution on the specified value. If the specified value contains the syntax
     ...
     * properties override system properties. </p>
     *
     ...
     * @throws IllegalArgumentException If there was a syntax error in the property placeholder syntax or a recursive variable reference.
     *
     */
