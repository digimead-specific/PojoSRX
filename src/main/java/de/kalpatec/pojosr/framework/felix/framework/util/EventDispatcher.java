...
public class EventDispatcher {
	...
	public Filter addListener(BundleContext bc, Class clazz, EventListener l, Filter filter) {
		...
		synchronized (this) {
			...
			if (clazz == FrameworkListener.class) {
				...
				/*
				 * Object sm = System.getSecurityManager(); if (sm != null) {
	...
	public void fireBundleEvent(BundleEvent event) {
		...
		// if we have hooks.
		Set<BundleContext> whitelist = createWhitelistFromHooks(event, event.getBundle(), listeners, syncListeners, org.osgi.framework.hooks.bundle.EventHook.class);
	...
	private Map<BundleContext, List<ListenerInfo>> filterListenersUsingHooks(ServiceEvent event, Framework felix, Map<BundleContext, List<ListenerInfo>> listeners) {
		Set<ServiceReference<org.osgi.framework.hooks.service.EventHook>> ehs = m_registry.getHooks(org.osgi.framework.hooks.service.EventHook.class);
		if ((ehs != null) && !ehs.isEmpty()) {
			// Create a whitelist of bundle context for bundle listeners,
			// if we have hooks.
			Set<BundleContext> whitelist = createWhitelistFromHooks(event, felix, listeners, null, org.osgi.framework.hooks.service.EventHook.class);

			// If we have a whitelist, then create copies of only the
			// whitelisted
			// listeners.
			if (whitelist != null) {
				Map<BundleContext, List<ListenerInfo>> copy = new HashMap<BundleContext, List<ListenerInfo>>();
		...
		Set<ServiceReference<org.osgi.framework.hooks.service.EventListenerHook>> elhs = m_registry.getHooks(org.osgi.framework.hooks.service.EventListenerHook.class);
		if ((elhs != null) && !elhs.isEmpty()) {
			...
			for (Entry<BundleContext, List<ListenerInfo>> entry : listeners.entrySet()) {
				Collection shrinkableCollection = new ShrinkableCollection(new ArrayList(entry.getValue()));
				shrinkableMap.put(entry.getKey(), (Collection<ListenerHook.ListenerInfo>) shrinkableCollection);
			}
			...
			// TODO: OSGi R4.3 - Should check and only do this if there was a
			// change.
			// Also, it is inefficient to have to create new lists for the
			// values.
			...
			for (Entry entry : shrinkableMap.entrySet()) {
				if (!((Collection) entry.getValue()).isEmpty()) {
					newMap.put((BundleContext) entry.getKey(), new ArrayList<ListenerInfo>((Collection) entry.getValue()));
				}
			}
			listeners = newMap;
		}
	...
	private <T> Set<BundleContext> createWhitelistFromHooks(EventObject event, Bundle bundle, Map<BundleContext, List<ListenerInfo>> listeners1, Map<BundleContext, List<ListenerInfo>> listeners2, Class<T> hookClass) {
	...
	private static Map<BundleContext, List<ListenerInfo>> removeListenerInfo(Map<BundleContext, List<ListenerInfo>> listeners, BundleContext bc, int idx) {
		...
		if (infos != null) {
			...
			infos.remove(idx);
			if (!infos.isEmpty()) {
				// Put the listeners back into the copy of the map and return
				// it.
				copy.put(bc, infos);
			}
			return copy;
